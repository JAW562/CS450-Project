...
void kmain(void)
{
	...
	// as it describes what is about to happend.
	...
	// add_byte_to_ring(device1, 'c');

	// read_from_ring(device1, device1->ring_buffer, sizeof(device1->ring_buffer));
	...
	createProc("comhand", (int)comhand, 0, 9);

	createProc("idle", (int)sys_idle_process, 0, 0);
